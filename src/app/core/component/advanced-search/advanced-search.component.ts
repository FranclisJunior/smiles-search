...
export class AdvancedSearchComponent extends BaseSearchComponent implements OnInit {
    ...
    onSearchSubmit(): void {
        ...
        Object.keys(formValue).forEach(key => {
            ...
            if ((value !== null && value !== '') || value === false) {
                const property = this.mapProperties[key];

                filter.push(this.makeFilter(key, value, property.type, property.comparison));
            }
        });
        this.submitEndCloseMenu(filter);
    }

    makeFilter(field, value, type, comparison): { field: String; value: Object; comparison: String } {
        return {
            field: field,
